...
4. *   **強制啟動 MFA (Mandatory MFA Enrollment)**：
    ...
    *   **對象：所有帳戶**。包含本地帳號、Legacy 帳號、AD 帳號與外部 SSO 帳號。採取 **Zero Trust** 原則，外部傳入的驗證狀態不自動繼承，必須符合本系統之 MFA 規範。
...
6. *   **擴充性 (Extensibility)**：
    *   架構設計支援未來整合外部 SSO (Google/MS)。在 External Callback 流程中，可透過 Claims Mapping 決定是否將外部 MFA 聲明信任並映射至本地 `amr`（預設為不信任，需手動開啟）。
